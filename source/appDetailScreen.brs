...
Function showDetailScreen(screen As Object, showList As Object, showIndex as Integer) As Integer
    ...
    'print "showDetailsScreen"
    '`print showList[showIndex].Title
    globals = getGlobalAA()
    globals.analytics.trackEvent("Video Details", "Entered", showList[showIndex].Title, "1", [])
    ...
    while true
        ...
        if type(msg) = "roSpringboardScreenEvent" then
            if msg.isScreenClosed()
                ...
                print "ButtonPressed"
                'UA_trackEvent("Roku","Video playback start","","") 
                globals = getGlobalAA()
                globals.analytics.trackEvent("Video playback", "Started", "", "", [])
                if msg.GetIndex() = 1
                    'print "showList[showIndex].ContentId"
                    'print showList[showIndex].ContentId
                    PlayStart = RegRead(showList[showIndex].ContentId, "ResumeFrom")
                    
                    if PlayStart <> invalid then
                        'print "PlayStart"
                        showList[showIndex].PlayStart = PlayStart.ToInt()
                        'print "showList[showIndex].PlayStart "
                        'print showList[showIndex].PlayStart 
                    else
                        print "PlayStart is invalid"
                    endif
                    showVideoScreen(showList[showIndex])
                endif
                if msg.GetIndex() = 2
                    showList[showIndex].PlayStart = 0
                    'print showList[showIndex]
                    showVideoScreen(showList[showIndex])
                endif
