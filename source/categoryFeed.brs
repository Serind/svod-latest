...
Function InitCategoryFeedConnection() As Object
    ...
    conn.UrlPrefix = "http://exploitation.tv"
    'conn.UrlPrefix = "http://sandbox.exploitation.svod.co/roku/api/views"
    conn.UrlCategoryFeed = conn.UrlPrefix + "/categories_v1r62.xml"

    conn.Timer = CreateObject("roTimespan")

    conn.LoadCategoryFeed    = load_category_feed
    'conn.LoadActionFeed = load_action_feed
...
Function get_category_names()
    ...
    categoryNames.Push("Action")
    categoryNames.Push("Drama")
...
'******************************************************************
'** Given a connection object for a genre feed, fetch,
...
'******************************************************************
Function load_action_feed(conn As Object) As Dynamic

    http = NewHttp(conn.UrlGenreAction)

    Dbg("action feed url: ", http.Http.GetUrl())
    ...
    m.Timer.Mark()
    if xml.item = invalid then
        print "no item tag"
        return invalid
    endif

    if islist(xml.item) = false then
        print "invalid feed body"
        return invalid
    endif

    print "category getName: " + xml.item[0].GetName()
    
    if xml.item[0].GetName() <> "item" then
        print "no initial item tag"
        return invalid
    endif
...
Function load_category_feed(conn As Object) As Dynamic
    ...
    xml=CreateObject("roXMLElement")
    if not xml.Parse(rsp) then
         print "Can't category parse feed"
        return invalid
    endif
    Dbg("Parse Took: ", m.Timer)
